...
\subsubsection{Mehrfache Übertragung}
Im Abschnitt \ref{sec:Grundlagen:sec:Single-Revocation} wurde beim Single-Revocation-Schema das Problem der Mehrfachausführung erklärt.

\subsection{Unerlaubte Empfängermenge $\mathcal R$}
Sollte zu einem beliebigen Zeitpunkt $t' < t$ unerlaubte Empfänger existieren, fehlen auch jedem erlaubten Teilnehmer ($t-t'$) Stützstellen um mit Hilfe einer Polynominterpolation das Interpolationspolynom vom Grad $t$ zu bestimmen. Dieses Problem kann vom Gruppencontroller behoben werden:\\
...
\hspace*{0.45cm}Die Revocation-Nachricht hat eine Länge von $O(t)$. Genauer gesagt enthält die Nachricht ($t+1$) Elemente aus $Z^*_p$ und $t$ Elemente aus $Z_q$, wobei $|Z_q|$ beträchtlich kleiner als $|Z^*_p|$ sein kann. Üblicherweise sind $|Z_q|=160$ und $|Z^*_p|=1024$.\\
\hspace*{0.45cm}Der Gruppencontroller muss, im Gegensatz zu einem Teilnehmer, etwas mehr Speicherkapazität aufweisen. Dieser speichert das generierte Polynom $P$ und alle vergebenen Stützstellen, um eine doppelte Zuweisung zweier Teilnehmer zu vermeiden. Zudem müssen alle unerlaubten Empfänger markiert werden, um diese dann in der Nachricht übermitteln zu können.

\subsection{Sicherheit}
Das Verfahren von M. Naor und B. Pinkas kann wiederholt für Revocations verwendet werden, solange zu jedem Zeitpunkt bis zu maximal $t$ unerlaubte Empfänger davon abgehalten werden, den Gruppenschlüssel zu erfahren.
Das Verfahren ist sicher bis auf eine Vereinigung von maximal $t$ Empfängern. Solch ein Zusammenschluss von Benutzern kann nicht zwischen dem Gruppenschlüssel und einem zufälligen Wert unterscheiden.

\begin{proof}[\textbf{Beweisidee aus \cite{np00}}]
\hspace*{0.45cm}Der Beweis basiert auf der DDH-Annahme. Aus Gründen der Übersichtlichkeit wird der Beweis zunächst für den Fall $t=1$ gezeigt.\\
