...
%% ==============================
In diesem Abschnitt soll gezeigt werden, dass die Lagrange-Interpolation nicht die einzige Möglichkeit für die Bestimmung des Interpolationspolynoms ist. Das Ziel in diesem Kapitel wird sein, die Newton-Interpolation im Naor-Pinkas-Verfahren gleichermaßen erfolgreich anzuwenden. Der Vergleich beider Interpolationen wird in Abschnitt \ref{ch:np-revocation-schema:sec:fazit-all} näher beschrieben.
...
\hspace*{0.45cm}Hierzu sei $P$ wieder ein vom Gruppencontroller zufällig generiertes Polynom vom Grad $t$ über $\mathbb Z_q$ und $K_{u_i} = <I_{u_i}, P(I_{u_i})>$ die über einen privaten Kanal versendete persönliche Information des Empfängers $u_i$ mit $i \in [1,\dots,|\mathcal N|]$. Ferner wählt der Gruppencontroller vor der Übertragung der Nachricht ein zufälliges $r \in \mathbb Z_q$ und setzt $g^{rP(0)}$ als neuen Schlüssel $\mathcal S$. Im nächsten Schritt versendet der Gruppencontroller folgenden Inhalt in Klartext an alle Empfänger $u \in \mathcal N$:
\[
<g^r, g^{rP(I_{u_1})}, g^{rP(I_{u_2})}, \dots ,g^{rP(I_{u_t})}, I_{u_1},  I_{u_2}, \dots,  I_{u_t}>.
\]
Mit Hilfe der Newton-Interpolation soll jeder Empfänger $u_0 \in \mathcal N \setminus \mathcal R$ das vom Gruppencontroller generierte Polynom eindeutig bestimmen können. Hierzu soll der Teilnehmer $u_0$ das Polynom an der Stelle $\mathcal S=(g^r)^{P(0)}$ bestimmen können:
\begin{align*}
\mathcal S & = (g^r)^{P(0)}\\
...
\label{eq:newton-aufwand-gesamt}
  & 1E +\sum_{i=1}^{t} \left( i\cdot M + E \right) +1E+\sum_{i=1}^{t} \left( i\cdot \left( iA + \left( i-1 \right) M + 1D +1E \right) +(2^i-1)\cdot D \right)\notag\\
  & = (t+2)\cdot E+ \sum_{i=1}^{t} \left( i\cdot (iA+iM+1D+1E)+(2^i-1)\cdot D\right)
\end{align}
...
Die Newton-Interpolation ist für den praktischen Einsatz eher geeignet, da diese sich numerisch sehr einfach anwenden lässt und im Vergleich zur Lagrange-Interpolation mit weniger Aufwand zum gleichen Ergebnis kommt.\\
...
\hspace*{0.45cm}Zudem sind die Newton-Basisfunktionen nicht von den Stützwerten abhängig. Daher können in einem Zeitraum ohne Veränderung der Menge $\mathcal R$ die Werte beim Empfänger gespeichert werden, sodass diese nicht erneut berechnet und übertragen werden müssen. Aufgrund dieser Eigenschaft lässt sich der Aufwand auf den Wert aus \eqref{eq:newton-aufwand-koef} reduzieren. Unter anderem entfallen in diesem speziellen Fall die teuren Exponentenberechnungen.\\
\hspace*{0.45cm}Jedoch besitzt die Newton-Interpolation den Nachteil, dass viele Zwischenergebnisse für die Berechnung der Koeffizienten gespeichert werden müssen. Dies kann vor allem bei zustandslosen Empfängern mit beschränkter Speichermöglichkeit zu einem Problem führen.
