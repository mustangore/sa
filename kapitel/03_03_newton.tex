...
%% ==============================
Um den Berechnungsaufwand der Newton-Interpolation im Naor-Pinkas-Schema ermitteln zu können, werden die jeweiligen Berechnungsschritte eines Empfängers genauer analysiert. Die Formel für die Berechnung einer einzelnen Newton-Basisfunktion für $i>0$ lautet:
\[
...
\\
Für den restlichen Aufwand müssen die Schritte genauer betrachtet werden, die bei der Umformung für die Berechnung der Koeffizienten notwendig sind. Für den Koeffizienten $c_0$ gilt:
\begin{align*}
...
\end{align*}
Aufgrund der bekannten Werte $g^{r\cdot P(I_{u_1})}$ und $g^{r\cdot P(I_{u_0})}$ beträgt der Aufwand innerhalb der Klammern 1 D. Zusätzlich werden für die Exponentenberechnung 1 E, 1 A und 1 D benötigt. Für $c_2$ erhalten wir mit Hilfe der Formel aus \eqref{eq:newton-ci-new}:
\begin{align*}
...
\begin{align*}
  (2^i-1)\, D + i\cdot (i\,A+(i-1)\,M+1\,D+1\,E).
\end{align*}
Daraus abgeleitet benötigt ein Empfänger $u_0$ für die gesamte Berechnung der Koeffizienten folgenden Aufwand:
\begin{align}
  \label{eq:newton-aufwand-koef}
  1\,E +\sum_{i=1}^{t} \left( (2^i-1)\,D+ i\cdot \left( i\,A + \left( i-1 \right)\,M + 1\,D +1\,E \right) \right)
\end{align}
...
\label{eq:newton-aufwand-gesamt}
  & 1\,E +\sum_{i=1}^{t} \left( i\,M + E \right) +1\,E+\sum_{i=1}^{t} \left( (2^i-1)\,D+ i\cdot \left( i\,A + \left( i-1 \right)\,M + 1\,D +1\,E \right) \right)\notag\\
  & = (t+2)\,E+ \sum_{i=1}^{t} \left( (2^i-1)\,D+ i\cdot (i\,A+i\,M+1\,D+1\,E)\right)\notag\\
  &= 2\,E + \sum_{i=1}^t \left((2^i-1)\,D+ i\cdot(i\,A+i\,M+1\,D+1\,E) +1\,E \right)
\end{align}
...
\hspace*{0.45cm}Dieser Vorteil ist beim Einsatz der Newton-Interpolation im Naor-Pinkas-Verfahren jedoch nicht möglich. Dies liegt daran, dass die Stützstellen $P(I_{u_i})$ für die rekursive Berechnung der Koeffizienten $c_i$ in \eqref{eq:newton_koeff_c_i} nicht bekannt sind. Ein Empfänger muss für alle Koeffizienten die Umformung aus \eqref{eq:newton-ci-new} vollziehen, was zum Gesamtaufwand aus \eqref{eq:newton-aufwand-gesamt} führt.\\
...
\hspace*{0.45cm}Da die Newton-Basisfunktionen nicht von den Stützwerten abhängig sind, können diese in einem Zeitraum ohne Veränderung der Menge $\mathcal R$ beim Empfänger gespeichert werden, sodass diese nicht erneut berechnet werden müssen. Aufgrund dieser Eigenschaft lässt sich der Aufwand um der Wert aus \eqref{eq:newton-aufwand-basis} reduzieren.\\
